...
ekfc_cr <- function(creatinine, age, female){
   Q <- ifelse(age <= 25 & female == 0, exp(3.200 + 0.259 * age - 0.543 * log(age) - 0.00763 * age ^ 2 + 0.0000790 * age ^ 3),
               ifelse(age <= 25 & female == 1, exp(3.080 + 0.177 * age - 0.223 * log(age) - 0.00596 * age ^ 2 + 0.0000686 * age ^ 3),
                      ifelse(age > 25 & female == 0, 80,
                             ifelse(age > 25 & female == 1, 62, NA))))
   
   return(ifelse(creatinine/Q < 1 & age <= 40, 107.3 * (creatinine/Q) ^ -0.322,
                 ifelse(creatinine/Q < 1 & age > 40, 107.3 * (creatinine/Q) ^ -0.322 * 0.990 ^ (age - 40), 
                        ifelse(creatinine/Q >= 1 & age <= 40, 107.3 * (creatinine/Q) ^ -1.132,
                               ifelse(creatinine/Q >= 1 & age > 40, 107.3 * (creatinine/Q) ^ -1.132 * 0.990 ^ (age - 40), NA)))))
}

ekfc_cys <- function(cystatin, age){
   Q <- if_else(age >50, 0.83+0.005*(age-50), 0.83)
   
   return(if_else(age >= 18 & age <= 40 & cystatin/Q < 1, 107.3 * (cystatin/Q) ^ -0.322,
                  if_else(age >= 18 & age <= 40 & cystatin/Q >= 1, 107.3 * (cystatin/Q) ^ -1.132, 
                          if_else(age > 40 & age <= 50 & cystatin/Q < 1, 107.3 * (cystatin/Q) ^ -0.322 * 0.990 ^ (age-40),
                                  if_else(age > 40 & age <= 50 & cystatin/Q >= 1, 107.3 * (cystatin/Q) ^ -1.132 * 0.990 ^ (age-40), 
                                          if_else(age > 50 & cystatin/Q < 1, 107.3 * (cystatin/Q) ^ -0.322 * 0.990 ^ (age-40),
                                                  if_else(age > 50 & cystatin/Q >= 1, 107.3 * (cystatin/Q) ^ -1.132 * 0.990 ^ (age-40), NA)))))))
}
...
plot_HRs <- function(.extracted_termplots, .outcome) {
   ...
   .plot <- .extracted_termplots %>%
      mutate(bold_line = case_when(predictor == "mGFR" ~ 1,
                                   TRUE ~ 0)) %>%
      ggplot(aes(x = x1, y = y1, group = predictor, color = predictor, fill = predictor, linewidth = factor(bold_line))) +
      geom_line() +
      geom_ribbon(aes(ymin = lower, ymax = upper, group = predictor), alpha = 0.1, color = NA) +     # area inside 95% CI
      geom_hline(yintercept = 1, linetype = "dashed", color = "black") + 
      labs(x = expression("Glomerular filtration rate, mL/min/1.73m"^2),
...
format_table <- function(.bound_termplots, .outcome) {
   .bound_termplots %>%
      mutate(x1 = scales::number(x1, accuracy = 1), 
             across(all_of(c("y1", "upper", "lower")), ~ scales::number(.x, accuracy = 0.01))) %>%
...
compute_fits <- function(outcome, predictor, covariates, data, .spline = "cubic", .linear_knots = "c(30, 60, 90)"){
   ...
   if (.spline == "linear"){
      .surv <- paste0("Surv(", .time, ",", outcome, ") ~ splines::bs(", predictor,", knots =", .linear_knots, ", degree = 1) +")
      
   } else if (.spline == "cubic"){
      .surv <- paste0("Surv(", .time, ",", outcome, ") ~ rcs(", predictor,",4) +")
   ...
   dat1 <- dat1 %>%
      mutate(predictor = names(ptemp)[[1]],
             predictor = factor(predictor, 
                                levels = c("mgfr", "ckd_epi_2021_cr", "ckd_epi_2012_cys", "ckd_epi_2021_cr_cys"),
                                labels = c("mGFR", 
                                           "eGFR[cr]",
                                           "eGFR[cys]",
                                           "eGFR[cr-cys]")))
...
plot_outcomes <- function(.predictor, outcome_element, outcome_clean, .covariates, .data, .ref, .trunc, .spline = "cubic"){
...
summarize_HR_CCA <- function(.predictor, outcome_element, outcome_name, .covariates, .data, .ref, .trunc, .spline = "cubic"){
   .predictor %>%
      map(~ compute_fits(outcome_element, .x, .covariates, .data, .spline)) %>%
      map(~ extract_termplot(.x, ref = .ref, .trunc)) %>%
      list_rbind() %>%
...
compute_fits_imp_data <- function(.outcome, .predictor, .covariates, .imp_data, .spline = "cubic", .linear_knots = "c(30, 60, 90)", .weights = NULL) {
   ...
   if (class(.imp_data) == "mids") {
      ...
      .range_predictor <- paste0(range(.imp_data[["data"]][[.imp_predictor]]), collapse = ",")
...
extract_each_termplotof_imp <- function(fit,ref,truncate) {
   ...
   dat1 <- dat1 %>%
      mutate(predictor = names(ptemp)[[1]],
             predictor = factor(predictor, 
                                levels = c("mgfr_s1", "ckd_epi_2021_cr_s1", "ckd_epi_2012_cys_s1", "ckd_epi_2021_cr_cys_s1"),
                                labels = c("mGFR", 
                                           "eGFR[cr]",
                                           "eGFR[cys]",
                                           "eGFR[cr-cys]")))
...
pool_termplots <- function(.extracted_termplots) {
   ...
   .pooled_termplots <- .extracted_termplots %>%
      ...
      mutate(yhat = mean(c_across(starts_with("y1"))),               # Pooled estimate: Q/m
             ...
             total_var = within_imp_var + between_imp_var + between_imp_var /length(.extracted_termplots), # Total variance V = W + B + B/m
      ...
      select(x1, y1 = HR_centered, lower = HR_CI_low, upper = HR_CI_high, predictor)
...
plot_imputed_outcomes <- function(.predictor, outcome_element, outcome_name, .covariates, .imp_data, .ref, .trunc, .spline = "cubic", .linear_knots = "c(30,60,90)", .weights = NULL){
...
summarize_HR_MICE <- function(.predictor, outcome_element, outcome_name, .covariates, .imp_data, .ref, .trunc, .spline = "cubic", .linear_knots = "c(30,60,90)", .weights = NULL){
